...
LD_FLAGS = -X github.com/hdac-io/tendermint/version.GitCommit=`git rev-parse --short=8 HEAD` -s -w
BUILD_FLAGS = -mod=readonly -ldflags "$(LD_FLAGS)"
HTTPS_GIT := https://github.com/hdac-io/tendermint.git
...
draw_deps:
	...
	@goviz -i github.com/hdac-io/tendermint/cmd/tendermint -d 3 | dot -Tpng -o dependency-graph.png
.PHONY: draw_deps
...
format:
	...
	find . -name '*.go' -type f -not -path "*.git*"  -not -name '*.pb.go' -not -name '*pb_test.go' | xargs goimports -w -local github.com/hdac-io/tendermint
.PHONY: format
