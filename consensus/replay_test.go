...
func testHandshakeReplay(t *testing.T, config *cfg.Config, nBlocks int, mode uint, testValidatorsChange bool) {
	...
	state = buildTMStateFromChain(store, config, stateDB, state, chain, nBlocks, mode)
	...
	if nBlocks > 0 {
		...
		buildAppStateFromChain(store, proxyApp, stateDB1, genisisState, chain, nBlocks, mode)
	}
...
func applyBlock(store sm.BlockStore, stateDB dbm.DB, st sm.State, blk *types.Block, proxyApp proxy.AppConns) sm.State {
	testPartSize := types.BlockPartSizeBytes
	blockExec := sm.NewBlockExecutor(store, stateDB, log.TestingLogger(), proxyApp.Consensus(), mempool, evpool)
...
func buildAppStateFromChain(store sm.BlockStore, proxyApp proxy.AppConns, stateDB dbm.DB,
	...
	case 0:
		for i := 0; i < nBlocks; i++ {
			block := chain[i]
			state = applyBlock(store, stateDB, state, block, proxyApp)
		}
	case 1, 2:
		...
		if mode == 2 {
			...
			state = applyBlock(store, stateDB, state, chain[nBlocks-1], proxyApp)
		}
	}

}

func buildTMStateFromChain(store sm.BlockStore, config *cfg.Config, stateDB dbm.DB, state sm.State, chain []*types.Block, nBlocks int, mode uint) sm.State {
	...
	case 1, 2:
		...
		applyBlock(store, stateDB, state, chain[len(chain)-1], proxyApp)
	}
...
func (bs *mockBlockStore) SaveBlock(block *types.Block, blockParts *types.PartSet, seenCommit *types.Commit, blockCommitLength int64) {
}
